...
void Scene_Play::loadLevel(const std::string &fileName)
{
    ...
    //     0
    // );
    // NOTE: You final code should position the entity with the grid x,y position
    // read from the file:
...
void Scene_Play::sMovement()
{
    ...
    {
        if (m_player->getComponent<CInput>().canJump && m_player->getComponent<CInput>().stopJump)
        {
            m_player->getComponent<CInput>().canJump = false;
            m_player->getComponent<CInput>().stopJump = false;
    ...
    for (auto e : m_entityManager.getEntities())
    {
        if (e->hasComponent<CGravity>())
        {
            ...
            if (e->tag() == "player" && v.y > 0)
            {
                v.y += 2 * e->getComponent<CGravity>().gravity;
            }
...
void Scene_Play::sLifespan()
{
    ...
    if (m_entityManager.getEntities("bullet").size() == 0)
    {
...
void Scene_Play::sDoAction(const Action &action)
{
    ...
    else if (action.type() == "END")
    {
        if (action.name() == "JUMP")
        {
            ...
            m_player->getComponent<CInput>().stopJump = true;
        }
        else if (action.name() == "DOWN")
        {
