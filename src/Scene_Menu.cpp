...
void Scene_Menu::init()
{
    ...
    m_menuText.setPosition(
        m_game->window().getSize().x / 2.0 - titleSize * (m_title.length() + 1) / 2.0,
        titleSize * 3);
    ...
    for (int i = 0; i < m_menuStrings.size(); i++)
    {
        ...
        text.setPosition(
            m_game->window().getSize().x / 2.0 - 26 * (m_menuStrings[i].length() + 1) / 2.0,
...
void Scene_Menu::sRender()
{
    ...
    // draw help
    sf::Text help("W:UP S:DOWN SPACE: PLAY ESC:BACK/QUIT",
                  m_game->assets().getFont("Mario"),
                  26);
    help.setFillColor(sf::Color::Black);
    help.setPosition(
        m_game->window().getSize().x / 2.0 - 26 * (help.getString().getSize() + 1) / 2.0,
